...
def main():
    ...
    for sector, etf in SECTOR_ETFS.items():
        try:
            hist = raw[etf] if etf in raw.columns.get_level_values(0) else raw[etf]
    ...
    for ticker in tickers:
        try:
            hist = raw[ticker] if ticker in raw.columns.get_level_values(0) else raw[ticker]
    ...
    # EDT offset: UTC-4; EST: UTC-5. Use UTC-4 for summer (approx)
    et_hour = hour_utc - 4
    if weekday >= 5:
        market_status = "closed"
    elif et_hour < 9 or (et_hour == 9 and minute_utc < 30):
    ...
    with open("data.js", "w") as f:
        ...
        f.write(f"const DATA = {json.dumps(payload, indent=2)};\n")
