...
def main():
    ...
    with open("data.js", "w", encoding="utf-8") as f:
        ...
        f.write(f"const DATA = {json.dumps(sanitized, indent=2)};\n")

    with open("data.json", "w", encoding="utf-8") as f:
        json.dump(sanitized, f, indent=2)

    print(f"data.js + data.json written — top pick: {top['ticker']} ({top['composite']:.1f}/100) | {total} stocks | status: {market_status}")
